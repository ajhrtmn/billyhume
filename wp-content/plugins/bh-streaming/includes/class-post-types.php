 ...
 *
 * - bh_track: one song. Artist stays a plain text field rather than a
 ...
 *   make sense here.
 * - bh_release: an album/EP/single grouping tracks together.
 * - bh_playlist: an ordered, curated set of tracks (owned by whoever
 ...
 *   pulled from a feed source become real bh_track posts flagged with
 *   _bhs_source = 'external', so the rest of the catalog/player code
 *   never needs to know the difference between a local and an
 *   aggregated track.
 */
class BHS_PostTypes {
    const MENU_PARENT = 'edit.php?post_type=bh_track';

    public static function register() {
        register_post_type('bh_track', [
            'labels' => [
        ...
        register_post_type('bh_playlist', [
            'labels' => [
        ...
        register_taxonomy('bhs_genre', 'bh_track', [
