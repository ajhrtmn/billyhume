...
Step shape: `['type' => 'assignment', 'title' => ..., 'instructions' => wp_kses_post(...), 'due_date' => 'Y-m-d' or null, 'submission_type' => 'audio'|'file'|'text', 'grading_mode' => 'pass_fail'|'numeric']` — `grading_mode` decided 2026-07-25 (see Section 5), picked per-assignment at creation time, not a site-wide setting.
...
**DECIDED (2026-07-25):** support both grading modes, not one or the other. Each assignment step (Section 3) carries its own `grading_mode` field (`pass_fail` or `numeric`), set when the assignment is created — an instructor picks whichever fits that specific assignment (a scale exercise might be pass/needs-revision; a graded performance might want a 0-100 score). `bhc_submissions` (Section 4) carries BOTH a `grade_pass` (nullable boolean) and `grade_numeric` (nullable int) column rather than one polymorphic "grade" field — only the one matching that assignment's `grading_mode` is ever written to, and the grading UI shows the matching control (toggle vs. number input) based on the assignment's own mode. Free-text feedback is written either way, every time — never optional, since "here's your grade" with no explanation is the exact opposite of what this whole feature is for.
...
### UI surface
Admin: a "Submissions to grade" screen, structurally identical to `BHF_Queue`'s reviewer view in `bh-feedback/includes/class-portal-panel.php` (open queue, "claimed by me" list, a grade form) — its own separate submenu, gated on the new `bhcore_grade_assignments` capability rather than sharing bh-feedback's screen (see Decided, below). Front end: the student sees grade + feedback on their own assignment step once graded (Section 4's "past attempts + feedback inline").

### Decided (2026-07-25)
- **Capability: separate, not shared.** bh-courses' grading queue gets its OWN capability — `bhcore_grade_assignments` — distinct from bh-feedback's `bhcore_review_submissions`. Someone can grade course homework without being a paid-feedback reviewer, or vice versa; the two are trust-scoped independently, each grantable on its own via `OUS_RoleAssignment`'s existing jobs UI (a new "Grader" job entry alongside Instructor/Reviewer/Designer/CRM Manager). The claim/release/complete *code shape* is still copied from `BHF_Queue` (same reasoning as before — proven pattern, not proven-to-be-shared code), but the two queues are gated, and can be granted, completely independently.
- **Grading mode: both, chosen per-assignment.** See Section 3/4's `grading_mode` field — not a site-wide setting, not a single compromise format.

### Remaining open question for AJ
- Does a "needs revision" grade (pass/fail mode) automatically re-open the step for resubmission (i.e. does the student see an explicit "try again" state), or is that implicit in the assignment step simply still showing as incomplete? For numeric-mode assignments, is there a passing threshold that also unblocks progress (e.g. ≥70 counts as "done"), or does ANY numeric grade count as complete regardless of score, with the number being informational only?
...
Open questions from every section above are collected there rather than repeated here. The two most load-bearing ones from Section 5 are now settled (2026-07-25): bh-courses gets its own `bhcore_grade_assignments` capability (not shared with bh-feedback's `bhcore_review_submissions`), and grading supports both pass/needs-revision AND numeric modes, picked per-assignment. What's left open across the rest of the doc — scheduling's single-vs-multi-instructor question (Section 1), whether a cross-course progress view is actually needed in practice (Section 2), per-student vs. shared assignments (Section 3), file-size ceilings for non-audio submissions (Section 4), and the pass/needs-revision resubmission + numeric-threshold behavior (Section 5) — are all smaller, lower-stakes calls that can be made as each piece is actually built, not blockers to starting Section 2 (the cheapest, most-proven-value item).
