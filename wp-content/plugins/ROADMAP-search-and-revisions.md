...
5. ✅ **SHIPPED** — `BHY_Style` (Design Suite's global design tokens): a single site-wide config, snapshotted on every save, with a real Version History panel + Restore right in the Design Suite page. Runtime-verified with a real color-change/restore cycle.
   - **Portal layouts checked, confirmed not yet a real consumer to wire** — there's no admin-editable layout config today (panels are hardcoded via the `bhi_portal_panels` filter, nothing an admin can reorder/save) — building a revisions consumer for a config that doesn't exist would be building for a hypothetical. Still open, honestly, not silently dropped.
...
**Both features fully shipped.** Search: shell + UI + all four originally-planned content providers (courses, contests, registry artists, streaming tracks), plus a deliberate, documented exclusion of bh-crm on privacy grounds. Revisions: shell + shared UI fragment + real consumers (bh_lesson/bh_course via native WP revisions, bh_contest/BHM_Tiers/BHY_Style via `OUS_Revisions` directly), all runtime-verified including full save/restore cycles. Only remaining open item: a portal-layout revisions consumer, blocked on portal layouts not being a real, admin-editable thing yet.
