...
**Both features fully shipped.** Search: shell + UI + all four originally-planned content providers (courses, contests, registry artists, streaming tracks), plus a deliberate, documented exclusion of bh-crm on privacy grounds. Revisions: shell + shared UI fragment + real consumers (bh_lesson/bh_course via native WP revisions, bh_contest and BHM_Tiers via `OUS_Revisions` directly), all runtime-verified including full save/restore cycles. Only remaining open item in either section: `BHY_Style`/portal-layout revisions consumers, whenever picked up next.
