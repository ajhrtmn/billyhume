...
3. ✅ **SHIPPED IN FULL (3 of 4 originally-planned providers; the 4th deliberately excluded)** — bh-contest (published contests only, linking to the real contest page), bh-registry (reuses the same `active`/verified-only gate its own public REST search already enforces), and bh-streaming (a new real `?bhs_track=ID` deep link the player SPA reads on load, closing the "no canonical URL" gap this doc originally flagged) — all three runtime-verified live.
...
5. ✅ **SHIPPED** — `BHY_Style` (Design Suite's global design tokens): a single site-wide config, snapshotted on every save, with a real Version History panel + Restore right in the Design Suite page. Runtime-verified with a real color-change/restore cycle.
6. ✅ **SHIPPED** — `OUS_PortalLayout`: the admin-editable portal layout config that was flagged as missing, built as its own real feature first (a "Portal Layout" admin page — priority number + hide checkbox per panel, no drag-and-drop dependency), then wired as an `OUS_Revisions` consumer (`object_type = 'portal_layout'`) exactly like the others. Runtime-verified: live nav reorder/hide confirmed at `/account/`, plus a full save→save→restore cycle across 3 versions.
7. Not in scope for v1: a visual diff view between two versions (a real, distinct feature layered on top of `get_version()` once basic restore is proven) — flagged, not built.
...
**Both features fully shipped, no open items remaining.** Search: shell + UI + all four originally-planned content providers (courses, contests, registry artists, streaming tracks), plus a deliberate, documented exclusion of bh-crm on privacy grounds. Revisions: shell + shared UI fragment + real consumers (bh_lesson/bh_course via native WP revisions, bh_contest/BHM_Tiers/BHY_Style/portal layout via `OUS_Revisions` directly), all runtime-verified including full save/restore cycles.
