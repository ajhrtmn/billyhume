...
bootstrap and gets a plain text field in the Design Suite's "Plugin fonts"
group, persisted under `bhy_style_settings['custom_fonts']`, emitted as
`--bh-custom-font-<key>` by `inline_css()`. `class-style-gallery.php` renders
the new group and its live-preview JS the same way the sliders already work.
`class-portal.php`'s "My Courses" portal-panel card now shows the course's own
Featured Image (falling back to an accent-gradient placeholder), matching the
`/courses/` catalog card — closes the gap where a course thumbnail only
showed on the public catalog, never in the logged-in "My Courses" list.
Refreshed bundled bh-courses.zip (0.16.29, the four Design Suite lesson-screen
sliders/font that consume this).

3.21.30 — Context-aware colour system (Phase 1: resolver + audit). New
`BHY_Color` (includes/class-color.php) holds the pure, WP-free colour
maths — sRGB/OKLCH, a WCAG 2.x ratio, and an APCA-W3 0.1.9 perceptual Lc
(three canonical APCA vectors verified: 63.1 / -107.9 / 106.0). The
WCAG `ink_on`/`contrast`/`relative_luminance` helpers moved here out of
`BHY_Style` (they were private, single-caller). New `BHY_Contrast`
(includes/class-contrast.php) derives every colour that DEPENDS on the
Design Suite seed colours from one recipe table (`BHY_Contrast::SPEC`):
the four accent tokens `BHY_Style::inline_css()` used to hand-roll
(`--bh-accent-muted-bg` / `-contrast` / `-hover` / `-pressed`, now
reproduced byte-for-byte — verified on the live theme, no visible
change) plus new context-aware roles — `--bh-on-accent` / `-surface` /
`-surface-2` / `-ui` / `-cat-1..8`, `--bh-text-muted`, `--bh-text-disabled`
(deliberately low-contrast so a disabled state still reads as disabled),
`--bh-border-strong`, `--bh-ui` / `-ui-hover` / `-ui-active`,
`--bh-focus`. APCA drives the derivation; WCAG 2.x is still measured for
the audit. Roles are generated by moving a seed's OKLCH lightness until
it hits an explicit contrast target (Leonardo-style), the vocabulary is
Radix-shaped. `inline_css()` and `entity_style_payload()` now go through
the one resolver; a non-autoload `bhy_contrast_palette` option caches the
global palette, refreshed on any `bhy_style_settings` write via
`update_option_`/`add_option_` (catches gallery save, revision restore
and REST with no edits to those paths) and snapshotted through
`OUS_Revisions`. Debug Tools → "Contrast Audit" (Monitoring group,
safe-in-production) shows every derived role and every raw seed pairing
measured against the current theme, plus a pass/fail matrix over all 26
built-in presets — all 26 clean on derived roles; the only flags are raw
seed pairings (accent-as-text, seed border) the audit is meant to
surface. New Test Runner suite `bhy-contrast` (includes/class-contrast-
test-suite.php), 69 assertions: metric vectors, OKLCH round-trips,
cross-theme resolver invariants (state ladder monotonic, body > muted >
disabled), byte-for-byte legacy reproduction, and a calibration snapshot
pinning the default palette. `assets/ts/contrast-core.ts` →
`assets/js/contrast-core.js` mirrors the resolver for the Design Suite
live preview, fed `BHY_Contrast::spec()` via `wp_localize_script` — one
spec, two engines, PHP↔JS output verified identical.
`BHY_Style::safe_color()` also now accepts `oklch()`/`oklab()` and a
plain sRGB `color-mix()`. Runtime-verified against the local WP install
(HTTP 200, full suite green, live `:root` block confirmed unchanged for
the four legacy tokens).
...
3.21.21 — Refresh bundled bh-courses.zip (0.16.23).
...
returns 200 with a real token (player.mediadelivery.net 403s a bare
request). Refresh bundled bh-courses.zip (0.16.12).
...
3.20.3 — Refresh bundled/bh-contest.zip (3.15.2, Contest Library card links).
