...
> **Correction (doc-cleanup pass, 2026-07-26): the standalone builder GUI this doc designs (§4, and the `BH_Element_Builder`/`BH_Element_Prefab`/`BH_Component_Studio` classes described in the status notes below) was built, then deliberately DELETED in a later pass** (`own-ur-shit.php`'s own changelog, ~line 1127: "PAGE-BUILDER-DELETE-KEEP-AUDIT.md (2026-07-13)... a bespoke per-Component HTML/CSS/JS block is still a bespoke editing mechanism where typed, native Gutenberg block types with real `render_callback()`s are simpler and more idiomatic" — WordPress's own native synced Patterns replaced the custom Prefab/instance system). `PAGE-BUILDER-DELETE-KEEP-AUDIT.md` itself no longer exists in this repo (deleted before this cleanup pass, its own reasoning already summarized in that changelog comment). **What's still current and accurate:** §1-3 below — `BH_Element`/`BH_Element_Data` (the data model + `render_slot()` render engine) are real, live, and unchanged — real pages in bh-contest, bh-crm, bh-courses, and own-ur-shit's own dashboard/portal all render through them today. For the CURRENT state of the Design Suite's actual editing UI (which replaced the GUI this doc describes), read `DESIGN-SUITE-UNIFICATION-PLAN.md` instead — that doc is this one's direct successor and reflects what's actually built now.
