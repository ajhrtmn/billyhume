...
add_action('wp_enqueue_scripts', function () {
    ...
    $has_listen   = has_shortcode($post->post_content, 'bh_listening_party');
    $has_reveal   = has_shortcode($post->post_content, 'bh_results_reveal');
    if (!$has_player && !$has_listen && !$has_reveal) return;

    // Shared by all three front-end shortcodes — same fonts, same
    // stylesheet, same theme variables (including any per-contest
    // override), so a Listening Party or Results Reveal page always
    // matches whatever look the main player has, automatically.
    ...
    if ($has_player) {
        ...
        wp_localize_script('bh-player', 'BHData', [
            ...
            'brand'    => ['part1' => $brand['brand_part1'], 'part2' => $brand['brand_part2'], 'logoUrl' => BH_Settings::logo_url($brand)],
        ]);
    }

    if ($has_listen) {
        wp_enqueue_script('bh-listening', BH_URL . 'assets/js/listening.js', [], BH_VER, true);
        wp_localize_script('bh-listening', 'BHData', [
            'rest' => esc_url_raw(rest_url('bh/v1/')),
        ]);
    }
