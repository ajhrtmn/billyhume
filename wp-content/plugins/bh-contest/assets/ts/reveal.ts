...
(function () {
    ...
    function tone(freq: number, startAt: number, duration: number, gain: number): void {
        var ctx = getAudioCtx();
        if (!ctx) return;
    ...
    function playCueFor(type: string, justRevealedRank?: number): void {
        if (type !== 'category_reveal' && type !== 'overall_reveal') return;
        if (justRevealedRank === undefined) return;
        if (justRevealedRank === 1) playWinnerFanfare();
        else playRevealBlip();
    }
    ...
    function confettiBurst(): void {
        ...
        var ctx: CanvasRenderingContext2D = rawCtx;
    ...
    function animateReveal(type: string, justRevealedRank?: number): void {
        if (type !== 'category_reveal' && type !== 'overall_reveal') return;

        var entries = (stage as HTMLElement).querySelectorAll('.bh-reveal-entry');
        if (!entries.length) return;

        if (justRevealedRank === 1) confettiBurst();
    ...
    function markStalled(stalled: boolean): void {
        if (!liveBadge) return;
        liveBadge.classList.toggle('bh-reveal-live-stalled', stalled);
        if (liveLabel) liveLabel.textContent = stalled ? 'RECONNECTING' : 'LIVE';
    }
    ...
    function poll(): void {
        ...
        fetch(url).then(function (r) { return r.json(); }).then(function (data: BHRevealState) {
            ...
            // outage, not noise.
            missedPolls++;
            if (missedPolls >= 3) markStalled(true);
        });
    }
