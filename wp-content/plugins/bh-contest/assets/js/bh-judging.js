...
(function () {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        panel.addEventListener('input', (e) => {
            ...
            if (!target.matches('input[type="range"][data-criterion]'))
                return;
        ...
        function collectScores(entry) {
            ...
            entry.querySelectorAll('input[data-criterion]').forEach((input) => {
                ...
                if (criterion)
                    scores[criterion] = parseInt(el.value, 10) || 0;
            });
            return scores;
        }
        function save(entry, status, attempt = 0) {
            var _a, _b;
            const body = new URLSearchParams();
            body.set('contest_id', String(BHJudgeData.contestId));
            body.set('submission_id', (_a = entry.dataset.submissionId) !== null && _a !== void 0 ? _a : '');
            body.set('category', (_b = entry.dataset.category) !== null && _b !== void 0 ? _b : '');
            ...
            fetch(BHJudgeData.restUrl, {
                ...
                if (!res.success) {
                    if (typeof BHCoreToast !== 'undefined') {
                        BHCoreToast.show('Could not save score.', 'error');
                    }
                    else {
                ...
                if (statusEl)
                    statusEl.textContent = submitted ? 'Submitted' : 'Draft';
                if (submitBtn)
                    submitBtn.textContent = submitted ? 'Update submission' : 'Submit score';
                if (typeof BHCoreToast !== 'undefined') {
                    BHCoreToast.show(submitted ? 'Score submitted.' : 'Draft saved.', 'success');
                }
            })
                .catch(() => {
        ...
        panel.addEventListener('click', (e) => {
            const target = e.target;
            const entry = target.closest('.bh-judge-entry');
            if (!entry)
                return;
            if (target.classList.contains('bh-judge-save-draft'))
                save(entry, 'draft');
            if (target.classList.contains('bh-judge-submit'))
                save(entry, 'submitted');
        });
    });
})();
