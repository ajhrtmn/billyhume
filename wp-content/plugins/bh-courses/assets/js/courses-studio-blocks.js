    ...
    wp.blocks.registerBlockType('bhc/video', {
        ...
        edit: function (props) {
            ...
            var chapterRows = chapters.map(function (c, i) {
                ...
                }, String(playbackOrder.indexOf(i) + 1)), el('code', { style: { fontSize: '12px', padding: '2px 6px', background: '#f0f0f0', borderRadius: '3px' } }, fmtTime(c.time)), el('span', { style: { flex: 1 } }), el(wp.components.Button, {
                ...
                }), el('div', { style: { display: 'flex', gap: '8px', alignItems: 'flex-end' } }, el(wp.components.TextControl, {
                ...
                untitled ? el('p', { style: { margin: '4px 0 0', fontSize: '12px', color: '#8a6d00' } }, __('Needs a title — untitled chapters are discarded when you save.')) : null, isPastEnd ? el('p', { style: { margin: '4px 0 0', fontSize: '12px', color: '#8a6d00' } }, __('Starts after this video ends (') + fmtTime(previewDuration) + __(') — it won\'t appear on the seek bar.')) : null);
            });
            ...
            var annotationRows = annotations.map(function (a, i) {
                ...
                }, __('Set to preview')) : null), el(wp.components.SelectControl, {
            ...
            previewUrl ? el(wp.components.PanelBody, { title: __('Preview'), initialOpen: true }, el('video', {
            ...
            }), el('p', { className: 'description', style: { marginBottom: 0 } }, __('Scrub to a moment, then add a chapter or overlay — both pick up this timestamp.'))) : null, el(wp.components.PanelBody, { title: __('Chapters') + (chapters.length ? ' (' + chapters.length + ')' : ''), initialOpen: false }, el('p', { className: 'description', style: { marginTop: 0 } }, __('Markers on the player\'s seek bar plus a clickable list beneath the video, like YouTube.')), chapters.length
                ? chapterRows
            ...
            }, previewUrl ? __('+ Add chapter at ') + fmtTime(previewTime) : __('+ Add chapter'))), el(wp.components.PanelBody, { title: __('Overlays') + (annotations.length ? ' (' + annotations.length + ')' : ''), initialOpen: false }, el('p', { className: 'description', style: { marginTop: 0 } }, __('Note, Hotspot and Question pause the video at a timestamp and wait for a click. Banner is deliberately different — a caption that slides in and leaves on its own, without stopping playback.')), annotations.length ? annotationRows : el('div', {
                style: {
                    border: '1px dashed #ccc', borderRadius: '6px', padding: '16px',
                    textAlign: 'center', color: '#757575', fontSize: '13px', marginBottom: '10px',
                },
            }, __('No overlays yet.')), el(wp.components.Button, {
                variant: 'secondary',
