...
6. **Resolve the `#1DB954` question.** It is `--bh-success` on the front end, but the same literal is hardcoded in admin files across `bh-contest`, `bh-streaming`, `bh-registry`, and `the-self-hosted-self`. `--bh-success` is printed via `wp_head` and never reaches wp-admin, so those admin uses resolve to nothing shared. Decide: promote an admin twin, or converge on `--bhy-success`.
