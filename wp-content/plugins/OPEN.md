...
- **CHANGELOG.md behind Version: header, 3 plugins remaining** (found 2026-08-25 by `tools/check-changelog-drift.sh`, now advisory in CI) — `bh-contest` (header 3.7.33, changelog's newest entry 3.7.30), `bh-tickets` (1.0.3 vs 1.0.2), and `self-hosted-self-admin-skin` (0.62.0 vs 0.31.0 — the widest gap). `bh-courses` closed itself out 2026-08-26 as a side effect of the real 0.5.0 release (item 22) landing right on top of the drift, though the 3 versions between 0.4.86 and 0.4.91 it never recorded are still unrecovered, same as the others. Not backfilled: reconstructing accurate entries for versions already shipped needs the real history behind each bump, which no session since has recorded.
