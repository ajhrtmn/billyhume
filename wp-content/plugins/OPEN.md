...
7. **Systematic front-end audit, plugin by plugin.** IN PROGRESS as of 2026-08-26 (the-self-hosted-self 3.15.4) — no longer "never done." `tests/ux/public.spec.ts` now covers 20 real pages (courses catalog/single/gated-lesson, contest single, streaming single/library, storefront shop/product/cart/checkout/account, CRM public profile, blog/search/404/login) at all 6 widths. Two real bugs found and fixed this pass: `BHI_PublicProfile::profile_url()` was a genuinely dead link (pointed at `home_url('/')` with nothing rendering `[bh_profile]` there — fixed via `OUS_Pages`, same pattern bh-contest/bh-courses already use), and a long achievement badge overflowed the page at 375px (a `flex-direction:column` + `align-items:center` container not constraining its own child's width — root-caused by walking the real ancestor chain in a browser, not guessed). Also fixed a false-positive in the audit tool itself (`tests/ux/audit.ts`'s touch-target check couldn't see an invisible `::before`/`::after` hit-area extension, so it was misreporting two already-fixed elements as failing).

    Still genuinely open: contest **voting/reveal** specifically (only the single-contest page is audited, not the live voting/results flow), and this was a logged-OUT pass only — a logged-in audit (courses-in-progress, CRM admin views, portal panels) is separate, unstarted work. Both themes wasn't exercised either — this pass, like the ones before it, only ran the site's current active theme.
