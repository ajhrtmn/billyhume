...
7. ~~**Systematic front-end audit, plugin by plugin.**~~ **DONE for logged-out + subscriber-logged-in, 2026-08-26** (the-self-hosted-self 3.15.4/3.15.5) — no longer "never done." `tests/ux/public.spec.ts` covers 20 logged-out pages; new `tests/ux/logged-in.spec.ts` covers 5 more as a real, persistent subscriber-role test fixture (courses in progress, portal, contest, streaming, all while authenticated). Real bugs found and fixed: `BHI_PublicProfile::profile_url()` was a genuinely dead link (fixed via `OUS_Pages`); a long achievement badge overflowed the page at 375px (a `flex-direction:column`+`align-items:center` container not constraining its own child); the portal's active-nav-tab text used raw `--bh-accent` on a dark surface at 3.68:1, fixed to the same `color-mix(65% accent, 35% text)` blend already used elsewhere. Also fixed a false-positive in the audit tool itself (couldn't see invisible `::before`/`::after` hit-area extensions).

    Still genuinely open: contest **voting/reveal** specifically (only the static single-contest page is audited, not the live voting/results flow — this is now explicitly queued as its own upcoming UX pass, see the note at the end of this file), and CRM **admin** views (item 8 covers wp-admin generally but not a full bh-crm person-view sweep specifically). Both themes wasn't exercised on the front end either — only admin screens (item 8) got the `THEMES` loop this pass.
8. ~~**Unaudited admin screens.**~~ **DONE 2026-08-26** (the-self-hosted-self 3.15.5, bh-registry 0.1.21) — ran `tests/ux/admin.spec.ts` for the first time EVER with real credentials (the `test.skip(!hasCreds)` guard meant it had silently never executed before). Style Gallery/API Docs/Roles/Metrics were already in the list; added bh-registry Submissions+Peers, bh-streaming Pro Wizard, bh-monetization-woo tier settings + a real tier post-edit screen, and a real lesson post-edit screen for "quiz editor" (Test Runner has no standalone page — it's a Debug Tools section, already covered). Used a scoped, throwaway administrator test account, deleted immediately after.

    Found and fixed 3 real bugs the harness itself had never caught before, all verified via the real Playwright run: two AA contrast failures on Debug Tools that had been there at every screen width (`.ous-log-level-pill[data-level="warning"]` at 2.81:1, `.ous-log-meta` at 4.41:1), and a clipped monospace token field on bh-registry's admin page at 375px. Also fixed two harness bugs surfaced in the process: a stale test entry pointing at 'API Docs' as a standalone page (that page 403s by design — `class-api-docs.php`'s own docblock documents the exact WordPress page-hook-resolution failure at length, and deliberately never re-enables it; the real content is the already-covered Debug Tools section), and a `#wpadminbar`-visibility check that doesn't account for Gutenberg's normal fullscreen mode hiding it.
