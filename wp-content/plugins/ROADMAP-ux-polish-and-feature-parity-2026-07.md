...
10. **WYSIWYG shortcode-to-block conversion** (5a. above) — start with `bhm_buy` to prove the pattern, then expand to the rest of the list; genuinely independent of everything else in this doc, slot in whenever capacity allows rather than strictly after #9. 🔶 in progress — all of bh-monetization-woo's zero/single-attribute shortcodes done: `bhm_buy` (0.4.9), `bhm_tip_jar` (0.4.10), `bhm_tiers` (0.4.11), all via `wp.serverSideRender` (real live preview, not a mimic), all runtime-verified end to end. Remaining: `bhm_wallet` (bh-monetization-woo), `bh_contest_player` + others (bh-contest), `bh_streaming` (bh-streaming), `bh_courses` (bh-courses) — not yet converted.
