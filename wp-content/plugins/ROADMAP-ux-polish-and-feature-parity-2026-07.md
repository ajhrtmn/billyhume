...
10. **WYSIWYG shortcode-to-block conversion** (5a. above) — start with `bhm_buy` to prove the pattern, then expand to the rest of the list; genuinely independent of everything else in this doc, slot in whenever capacity allows rather than strictly after #9. 🔶 in progress — bh-monetization-woo done: `bhm_buy` (0.4.9), `bhm_tip_jar` (0.4.10), `bhm_tiers` (0.4.11). bh-contest done: `bh_contest_player`, `bh_results_reveal`, `bh_archive` (3.5.0) — caught and fixed a real regression along the way (the plugin's front-end asset-enqueue gate only checked `has_shortcode()`, so a block-authored page would have rendered an inert, never-hydrated container without a `has_block()` fix). bh-streaming done: `bh_streaming` → `bhs/player` (0.5.4) — applied the same `has_block()` fix preemptively before shipping, not after. All via `wp.serverSideRender`, all runtime-verified end to end including full live-browser hydration checks. Remaining: `bhm_wallet` (bh-monetization-woo, deferred — user-specific view needs more thought), `bh_courses` (bh-courses, the catalog shortcode) — not yet converted.
...
11. **CRM: tag chips, saved smart lists, notes/reminders, bulk actions** (Section 3) — bundle as one CRM depth pass once the higher-priority confirmed items above are done.
