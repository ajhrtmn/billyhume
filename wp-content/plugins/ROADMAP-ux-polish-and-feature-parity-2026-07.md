...
10. **WYSIWYG shortcode-to-block conversion** (5a. above) — start with `bhm_buy` to prove the pattern, then expand to the rest of the list; genuinely independent of everything else in this doc, slot in whenever capacity allows rather than strictly after #9. 🔶 in progress — `bhm_buy` done via `wp.serverSideRender` (real live preview, not a mimic), runtime-verified end to end. Remaining shortcodes (`bhm_tip_jar`, `bhm_tiers`, `bhm_wallet`, `bh_contest_player` + others, `bh_streaming`, `bh_courses`) not yet converted.
