...
10. **WYSIWYG shortcode-to-block conversion** (5a. above) — start with `bhm_buy` to prove the pattern, then expand to the rest of the list; genuinely independent of everything else in this doc, slot in whenever capacity allows rather than strictly after #9. 🔶 in progress — `bhm_buy` (0.4.9) and `bhm_tip_jar` (0.4.10) done via `wp.serverSideRender` (real live preview, not a mimic), both runtime-verified end to end. Remaining shortcodes (`bhm_tiers`, `bhm_wallet`, `bh_contest_player` + others, `bh_streaming`, `bh_courses`) not yet converted — paused here for now, pick up with `bhm_tiers` next.
